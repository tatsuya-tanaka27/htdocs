...
if($_SERVER["REQUEST_METHOD"]==="POST"){
	...
	if(!isset($_POST["name"]) || !strlen($_POST["name"])){
		...
		$result=mysqli_stmt_execute($sql);//$resultにはtrueかfalseのみ。結果はメモリ
		mysqli_stmt_store_result($sql);//メモリ上の結果の取得
		if(mysqli_stmt_num_rows($sql)!=0){//取得行数
...
<?php endforeach; ?>
			<li><a href="kaiinntouroku2.html">登録画面に戻る</a></li>
		</ul>
<?php else: ?>
		<p>登録完了しました</p>
<?php endif; ?>
