        ...
        try {
            ...
            if(chackParam($pmail, $ppass, $ptel)){
                ...
                if($checkRow != null){
                    ...
                    $sql2 = "insert into kaiin(mail, pass, sex, tel, birth) 
                        VALUES(:mail, :pass, :sex, :tel, :birth)";
                    $stmt->bindValue(":mail", $pmail, PDO::PARAM_STR);
                    $stmt->bindValue(":mail", $ppass, PDO::PARAM_STR);
                    $stmt->bindValue(":mail", $psex, PDO::PARAM_STR);
                    $stmt->bindValue(":mail", $tel, PDO::PARAM_STR);
                    $stmt->bindValue(":mail", $pbirth, PDO::PARAM_STR);
                    $pdo->query($sql2);
                    $_SESSION['mail'] = $pmail;
                    // リダイレクトを実行
