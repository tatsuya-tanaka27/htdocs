    ...
    if(empty($umail) || empty($upass) || empty($utel) || empty($ubirth)){
        ...
        try {
           ...
           // if(chackParam($pmail, $ppass, $ptel)){
                ...
                $stmt = $pdo->prepare('insert into kaiin2(mail, pass, sex, tel, birth) 
                    VALUES(:mail, :pass, :sex, :tel, :birth)');
                $stmt->bindValue(":pass", $ppass, PDO::PARAM_STR);
                $stmt->bindValue(":sex", $psex, PDO::PARAM_STR);
                $stmt->bindValue(":tel", $ptel, PDO::PARAM_STR);
                $stmt->bindValue(":birth", $pbirth, PDO::PARAM_STR);
                $stmt->execute();
                $_SESSION['pass'] = $upass;
                $_SESSION['sex'] = $usex;
                $_SESSION['tel'] = $utel;
